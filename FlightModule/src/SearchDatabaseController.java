...
class SearchDatabaseController extends DatabaseController{

    boolean fetchSeats(Flight flight) {
    ...
    ArrayList<Flight> searchForFlights(Filter flightFilter) {
        if(flightFilter == null) {
            String query = getFlightQuery();
            return executeQuery(query, flightInitializer);
        }

        Utilities.Tuple<ArrayList<String>, ArrayList<Object>> filtersAndParameters = flightFilter.getFiltersAndParameters();
